...
class Vertex
{
    ...
    private :
        ...
        bool m_marque;
        std::vector<Vertex> liste_descendants;
        int m_numCC;
    ...
    public:
        ...
        Vertex (double value=0, VertexInterface *interface=nullptr, int idx=0, bool marque=false,int numCC=0, int _K=0) :
            m_value(value), m_interface(interface), m_idx(idx), m_marque(marque), m_numCC(numCC), m_K(_K)  {  }


        /// Vertex étant géré par Graph ce sera la méthode update de graph qui appellera
        /// le pre_update et post_update de Vertex (pas directement la boucle de jeu)
        /// Voir l'implémentation Graph::update dans le .cpp
        void pre_update(class Graph *g);
        void post_update(class Graph *g);
};
...
class OutilsInterface
{
    ...
    public :
        ...
        // Le constructeur met en place les éléments de l'interface
        // voir l'implémentation dans le .cpp
        OutilsInterface(int idx, int x, int y, std::string pic_name="", int pic_idx=0);
};
...
class Outils
{
    ...
    private :
        ...
        /// le POINTEUR sur l'interface associée, nullptr -> pas d'interface
        std::shared_ptr<OutilsInterface>m_interface = nullptr;
    ...
    public:
        ...
        /// Les constructeurs sont à compléter selon vos besoin...
        /// Ici on ne donne qu'un seul constructeur qui peut utiliser une interface
        Outils (double value=0, OutilsInterface *interface=nullptr) :
...
class Graph
{
    ...
    private :
        ...
        std::map<int, std::vector<int>> m_CC;
    ...
    public:
        ...
        /// Les constructeurs sont à compléter selon vos besoin...
        /// Ici on ne donne qu'un seul constructeur qui peut utiliser une interface
        Graph (GraphInterface *interface=nullptr, bool active=false) :
            m_interface(interface), m_active(active)  {  }
        ...
        ///Composants fortements connexes
        void CFC(int x);
        void dfs(int start, int pos, std::map<int, Vertex> m_vertices2);
        bool verif_marque();
        void redux_graph();
